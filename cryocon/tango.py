"""
Python source for the CryoConTempController and its commands.
...
class CryoConTempController(PyTango.Device_4Impl, cryocon.CryoCon):
    ...
    def _setup_standard_attributes(self):
        for attr in CryoConTempControllerClass.attr_list:
            ...
            if not hasattr(CryoConTempController,
                           method):  # respect method if already defined
                setattr(
                    CryoConTempController,
                    method,
                    CryoConTempController._standard_attr_state_machine)
...
# ------------------------------------------------------------------
    ...
    def delete_device(self):
        self.info_stream(
            '[Device delete_device method] for device %s' %
            self.get_name())
        try:
            ...
            # manufacturer but got no answer, so we have to live with this by
            # now.
...
# ------------------------------------------------------------------
    ...
    def init_device(self):
        ...
        # retrieve allowed transient property if present and reset
        # transient_errors
        self.transient_errors = 0
        if (self.AllowedTransientErrors == []) or (
                not isinstance(self.AllowedTransientErrors, int)):
        ...
        # try to initialize communications (set FAULT state and return if goes
        # wrong)
        try:
            ...
            msg = 'In %s::init_device() error while initializing communication: %s' % (
                self.get_name(), repr(e))
            self.error_stream(msg)
        ...
        try:
           ...
           # channels initialization
            ...
            self.read_temp_cmd = self.CMD_CH_TEMP_QUERY % (
                ','.join(self.channels_keys))
            self.read_loops_outputs_cmd = self.CMD_SEPARATOR.join(
                [self.CMD_LOOP_OUTPUT_QUERY % i for i in self.loops_keys])
            self.read_loops_rates_cmd = self.CMD_SEPARATOR.join(
                [self.CMD_LOOP_RATE_QUERY % i for i in self.loops_keys])
            self.read_loops_setpoints_read_cmd = self.CMD_SEPARATOR.join(
                [self.CMD_LOOP_SETPT_QUERY % i for i in self.loops_keys])
            self.read_loops_types_read_cmd = self.CMD_SEPARATOR.join(
                [self.CMD_LOOP_TYPE_QUERY % i for i in self.loops_keys])
            ...
            if (self.AutoLockFrontPanel != []) and (self.AutoLockFrontPanel):
                self.auto_lock = True
                cmd = (
                    self.CMD_SYS_LOCKOUT %
                    'ON') + self.CMD_SEPARATOR + self.CMD_SYS_LOCKOUT_QUERY
            ...
            cmd = (self.CMD_SYS_REMOTE_LED % 'ON') + \
                self.CMD_SEPARATOR + self.CMD_SYS_REMOTE_LED_QUERY
    ...
    def dev_state(self):
        ...
        try:
            ...
            result = self._communicate_raw(
                cmd, output_expected=True, strip_string=True)

            # check if communication was correct and reset transient_errors if
            # so
            ...
            control, lockout = [
                res.strip() for res in result.split(
            ...
            if self.control_enabled != control_now:
                ...
                if self.auto_lock:
                    msg = 'Control was supposed to be %s but it is %s. Please check!!!' % (
...
# ------------------------------------------------------------------
    ...
    def SetChannelUnit(self, argin):
        ...
        if not (channel in self.channels_keys):
            msg += 'Invalid channel: %s.Valid channels are: %s' % (
                channel, str(self.channels_keys))
        if not (unit in self.valid_units):
            msg += 'Invalid unit: %s.Valid units are: %s' % (
                unit, str(self.valid_units))
        if msg != '':
            self.error_stream(msg)
            PyTango.Except.throw_exception(
                'Bad parameter', msg, '%s::SetChannelUnit()' %
                self.get_name())

        # set and read back requested value and check it really changed (if not
        # raise exception)
        cmd = self.CMD_CH_UNIT % (
            channel, unit) + self.CMD_SEPARATOR + self.CMD_CH_UNIT_QUERY % (channel)
        unit_rb = self._communicate_raw(
        ...
        if unit != unit_rb_translated:
            msg = 'read back unit (%s) is different than the one set (%s).' % (
                unit_rb, unit)
            PyTango.Except.throw_exception(
                'Error', msg, '%s::SetChannelUnit()' %
                self.get_name())
