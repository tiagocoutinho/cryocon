"""
Python source defining common functions of all CryoCon devices
...
class CryoCon(object):
    ...
    CHANNELS = ['A', 'B', 'C', 'D']
    LOOPS = ['1', '2', '3', '4']
    # Kelvin, Celsius, Fahrenheit, Sensor depending
    VALID_UNITS = ['K', 'C', 'F', 'S']
    ...
    # case of M24C and '\xf4' for the M32. Reported to the manufacturer but
    # got no answer
    ...
    # setpoints seem to have some kind of preset values when not in K or S
    # units
...
# ------------------------------------------------------------------
    ...
    def _init_comms(self):
        ...
        if serial_class not in self.KNOWN_SERIAL_CLASSES:
            msg = 'Unknown serial class %s. Valid ones are: %s' % (
                serial_class, self.KNOWN_SERIAL_CLASSES)
            self.error_stream(msg)
            PyTango.Except.throw_exception(
                'Unknown serial class',
                msg,
                '%s::_init_comms()' %
                self.get_name())
        ...
        if serial_class == 'Serial':
            ...
            msg = 'Unsupported serial device class: %s. Valid ones are: %s' % (
                serial_class, str(self.KNOWN_SERIAL_CLASSES))
            self.error_stream(msg)
            PyTango.Except.throw_exception(
                'Communication init error',
                msg,
                '%s::_init_comms()' %
                self.get_name())

    def _communicate_raw_Serial(
            self,
            cmd,
        ...
        try:
            self.debug_stream(
                'In %s::_communicate_raw_Serial() command input: %s' %
                (self.get_name(), cmd))
            ...
            self.error_stream(
                'In %s::_communicate_raw_Serial() unexpected exception: %s' %
                (self.get_name(), traceback.format_exc()))
            raise
        finally:
            self.lock.release()

    def _communicate_raw_PySerial(
            self,
            cmd,
            output_expected=False,
            strip_string=True):
        ...
        try:
            self.debug_stream(
                'In %s::_communicate_raw_PySerial() command input: %s' %
                (self.get_name(), cmd))
            ...
            self.error_stream(
                'In %s::_communicate_raw_PySerial() unexpected exception: %s' %
                (self.get_name(), traceback.format_exc()))
            raise
        finally:
            self.lock.release()
...
# ------------------------------------------------------------------
    ...
    def _standard_attr_state_machine(self, req_type=None):
        ...
        if state in [
            ...
            self.error_stream(
                'In %s::_standard_attr_state_machine(): reached unexpected condition %s' %
                (self.get_name(), str(state)))
            return False
...
# ------------------------------------------------------------------
    ...
    def _get_channel_unit(self, channel):
        ...
        unit = self._communicate_raw(
            cmd, output_expected=True, strip_string=False)
        return unit
...
# ------------------------------------------------------------------
    ...
    def _update_loops_info(self):
        ...
        for loop in self.loops_keys:
            ...
            # special case: a loop may not have a source channel, but ONLY if
            # in manual mode
            if source not in self.channels_keys:
                if not (type_.upper() in ('MAN', 'OFF')):
                    msg = 'Loop %s is configured to be using channel %s, but the later is not being managed' % (
                        loop, source)
                    self.error_stream(msg)
    ...
    def read_Channels(self, attr):
        ...
        if (now - self.last_ch_read_time) > self.ch_read_validity:
            try:
                ...
                msg = 'In %s::read_Channels() error reading temperatures: %s' % (
                    self.get_name(), traceback.format_exc())
                self.error_stream(msg)
                PyTango.Except.throw_exception(
                    'Communication error',
            ...
            for idx, ch in enumerate(self.channels_keys):
                try:
                    ...
                    if read_temp == self.CH_OUT_OF_RANGE:
                        msg = 'Channel %s voltage out of range. Check sensor connections' % (
                            ch)
                    elif read_temp == self.CH_OUT_OF_LIMIT:
                        msg = 'Channel %s voltage in range, but measurement outside limits. Check sensor connections' % (
                            ch)
                    elif read_temp == self.CH_DISABLED:
                        msg = 'Channel %s seems to be disabled while it was not expected to be.' % (
                            ch)
                    else:
                        msg = 'Unexpected value returned from controller for %s: %s' % (
                            channel_name, read_temp)
                    self.error_stream(msg)
    ...
    def read_LoopsOutputs(self, attr):
        ...
        if (now - self.last_loops_outputs_read_time) > self.loops_outputs_read_validity:
            ...
            try:
                ...
                msg = 'In %s::read_LoopsOutputs() error reading outputs: %s' % (
                    self.get_name(), traceback.format_exc())
                self.error_stream(msg)
                PyTango.Except.throw_exception(
                    'Communication error',
    ...
    def read_Loop1Range_(self, attr):
        ...
        if (now - self.last_loops_ranges_read_time) > self.loops_ranges_read_validity:
            ...
            try:
                ...
                msg = 'In %s::read_Loop1Range_() error reading range: %s' % (
                    self.get_name(), traceback.format_exc())
                self.error_stream(msg)
                PyTango.Except.throw_exception(
                    'Communication error',
    ...
    def read_LoopsRates(self, attr):
        ...
        if (now - self.last_loops_rates_read_time) > self.loops_rates_read_validity:
            ...
            try:
                ...
                msg = 'In %s::read_LoopsRates() error reading ramping rates: %s' % (
                    self.get_name(), traceback.format_exc())
                self.error_stream(msg)
                PyTango.Except.throw_exception(
                    'Communication error',
...
# ------------------------------------------------------------------
    ...
    def read_LoopsSetPoints(self, attr):
        ...
        try:
            ...
            msg = 'In %s::read_LoopsSetPoints() error reading setpoints: %s' % (
                self.get_name(), traceback.format_exc())
            self.error_stream(msg)
            PyTango.Except.throw_exception(
                'Communication error',
                msg,
                '%s::read_LoopsSetPoints()' %
                self.get_name())
        ...
        for idx, loop_ in enumerate(self.loops_keys):
            try:
                ...
                read_setpoint = read_setpoint.strip(
                    expected_unit)  # remove unit character
                value = float(read_setpoint)
            except ValueError:
                value = float('NaN')
                msg = 'In %s::read_LoopsSetPoints() Error reading loop %s set point: %s' % (
                    self.get_name(), loop_, read_setpoint)
                self.error_stream(msg)
            except Exception as e:
                value = float('NaN')
                msg = 'In %s::read_LoopsSetPoints() unexpected exception: %s' % (
                    self.get_name(), traceback.format_exc())
                self.error_stream(msg)
            finally:
...
# ------------------------------------------------------------------
    ...
    def read_LoopsTypes(self, attr):
        ...
        try:
            ...
            msg = 'In %s::read_LoopsTypes() error reading loops types: %s' % (
                self.get_name(), traceback.format_exc())
            self.error_stream(msg)
            PyTango.Except.throw_exception(
                'Communication error',
...
# ------------------------------------------------------------------
    ...
    def _read_channels(self):
        result = self._communicate_raw(
            self.read_temp_cmd, output_expected=True)
...
# ------------------------------------------------------------------
    ...
    def _read_loops_setpoints(self):
        result = self._communicate_raw(
            self.read_loops_setpoints_read_cmd,
            output_expected=True,
            strip_string=False)
        results = result.split(self.RESULT_SEPARATOR)
        return results
...
# ------------------------------------------------------------------
    ...
    def _read_loops_types(self):
        result = self._communicate_raw(
            self.read_loops_types_read_cmd,
            output_expected=True)
        results = result.split(self.RESULT_SEPARATOR)
...
# ------------------------------------------------------------------
    ...
    def _write_loop_power_manual(self, loop, power):
        ...
        loop = str(loop)
        if loop not in self.loops_keys:
            msg += 'Invalid loop: %s. Valid loops are: %s' % (
                loop, str(self.loops_keys))
        if msg != '':
            self.error_stream(msg)
            PyTango.Except.throw_exception(
                'Bad parameter',
                msg,
                '%s::_write_loop_power_manual()' %
                self.get_name())
        ...
        cmd = self.CMD_LOOP_PMAN % (loop, str(
            power)) + self.CMD_SEPARATOR + self.CMD_LOOP_PMAN_QUERY % loop
        power_rb = self._communicate_raw(cmd, output_expected=True)
        power_rb = float(power_rb)
        if abs(power - power_rb) > self.DELTA_RB:
            msg = 'Written power %s differs from the one read back from instrument %s' % (
                str(power), str(power_rb))
            self.error_stream(msg)
            PyTango.Except.throw_exception(
                'Instrument error',
                msg,
...
# ------------------------------------------------------------------
    ...
    def _write_loop_range(self, loop, range_):
        ...
        if range_.upper() not in self.LOOP1_RANGES:
            msg += 'Invalid loop range: %r. Valid ranges are: %s' % (
                range_, str(self.LOOP1_RANGES))
        if msg != '':
            self.error_stream(msg)
            PyTango.Except.throw_exception(
                'Bad parameter',
        ...
        # request command
        cmd = self.CMD_LOOP_RANGE % (loop, range_.upper(
        )) + self.CMD_SEPARATOR + self.CMD_LOOP_RANGE_QUERY % loop
        range_rb = self._communicate_raw(cmd, output_expected=True)

        if range_.upper() != range_rb.upper():
            msg = 'Written loop range %s differs from the read back from instrument %s' % (
                str(range_), str(range_rb))
            self.error_stream(msg)
            PyTango.Except.throw_exception(
                'Instrument error',
                msg,
...
# ------------------------------------------------------------------
    ...
    def _write_loop_rate(self, loop, rate):
        ...
        # request command, including read_back
        cmd = self.CMD_LOOP_RATE % (loop, str(
            rate)) + self.CMD_SEPARATOR + self.CMD_LOOP_RATE_QUERY % loop
        ...
        if (rate != rate_rb) > self.DELTA_RB:  # 6 decimals precision
            msg = 'Written rate %s differs from the read back from instrument %s' % (
                str(rate), str(rate_rb))
            self.error_stream(msg)
            PyTango.Except.throw_exception(
                'Instrument error',
                msg,
    ...
    def _write_loop_set_point(self, loop, set_point):
        ...
        if msg != '':
            ...
            PyTango.Except.throw_exception(
                'Bad parameter',
                msg,
                '%s::_write_loop_set_point()' %
                self.get_name())

        # request command, including read_back
        cmd = self.CMD_LOOP_SETPT % (loop, str(
            set_point)) + self.CMD_SEPARATOR + self.CMD_LOOP_SETPT_QUERY % loop
        set_point_rb = self._communicate_raw(cmd, output_expected=True)
        expected_unit = self.channels[self.loops[loop].source].unit
        set_point_rb = set_point_rb.strip(
            expected_unit)  # remove unit character
        set_point_rb = float(set_point_rb)
        if abs(set_point - set_point_rb) > self.DELTA_RB_SETPT:
            msg = 'Written setpoint %s differs from the read back from instrument %s' % (
                str(set_point), str(set_point_rb))
            self.error_stream(msg)
            PyTango.Except.throw_exception(
                'Instrument error',
                msg,
...
# ------------------------------------------------------------------
    ...
    def _write_loop_type(self, loop, type_):
        ...
        loop = str(loop)
        if loop not in self.loops_keys:
            msg += 'Invalid loop: %r. Valid loops are: %s' % (
                loop, str(self.loops_keys))
        if type_.upper() not in self.loops_types:
            msg += 'Invalid loop control type: %r. Valid types are: %s' % (
                type_, str(self.loops_types))
        if msg != '':
            self.error_stream(msg)
            PyTango.Except.throw_exception(
                'Bad parameter',
        ...
        # do not allow change loop type to something different from manual or
        # off if associated channel is not being managed
        if not (
            self.loops[loop].source in self.channels_keys) and not (
            type_.upper() in (
                'MAN',
                'OFF')):
        ...
        # request command
        cmd = self.CMD_LOOP_TYPE % (loop, type_.upper(
        )) + self.CMD_SEPARATOR + self.CMD_LOOP_TYPE_QUERY % loop
        type_rb = self._communicate_raw(cmd, output_expected=True)
        if type_.upper() != type_rb.upper():
            msg = 'Written loop type %s differs from the read back from instrument %s' % (
                str(type_), str(type_rb))
            self.error_stream(msg)
            PyTango.Except.throw_exception(
                'Instrument error',
                msg,
